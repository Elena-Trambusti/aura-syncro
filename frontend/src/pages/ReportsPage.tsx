  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          <h1 className="text-2xl font-bold text-slate-800">Report & Contabilità</h1>
        ...
        <div className="flex items-center gap-2">
          ...
          <select value={selectedMonth} onChange={e => setSelectedMonth(+e.target.value)} className="border border-slate-200 rounded-xl px-3 py-2 text-sm bg-white">
      ...
      <div className="flex gap-2 flex-wrap">
        {([
          ...
          <button key={t.key} onClick={() => setActiveTab(t.key)}
            className={`px-4 py-2 rounded-lg text-sm font-medium transition-colors ${activeTab === t.key ? 'bg-orange-500 text-white' : 'bg-white border border-slate-200 text-slate-600 hover:bg-slate-50'}`}>
            {t.label}
          </button>
        ))}
      </div>
      ...
      {activeTab === 'pl' && (
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-2 xl:grid-cols-4 gap-4">
            {[
              { label: 'Fatturato', value: formatCurrency(s?.revenue || 0), icon: TrendingUp, color: 'bg-emerald-500', sub: `${s?.orders || 0} ordini` },
              { label: 'Food Cost', value: formatCurrency(s?.estimatedFoodCost || 0), icon: TrendingDown, color: 'bg-orange-500', sub: `${s?.foodCostPct || 0}% del fatturato` },
              { label: 'Utile Lordo', value: formatCurrency(s?.grossProfit || 0), icon: TrendingUp, color: (s?.grossProfit || 0) >= 0 ? 'bg-blue-500' : 'bg-red-500', sub: 'Ricavi - Food Cost' },
              { label: 'Utile Netto', value: formatCurrency(s?.netProfit || 0), icon: TrendingUp, color: (s?.netProfit || 0) >= 0 ? 'bg-purple-500' : 'bg-red-500', sub: `Costo personale: ${formatCurrency(s?.laborCost || 0)}` },
            ].map(c => (
          ...
          <div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
            <div className="bg-white rounded-2xl p-6 border border-slate-100 shadow-sm">
              <div className="flex items-center justify-between mb-4">
                <h3 className="text-base font-semibold text-slate-800">Dettaglio {MONTHS[selectedMonth]} {selectedYear}</h3>
                <button onClick={exportPL} className="flex items-center gap-1.5 text-xs text-orange-600 hover:text-orange-700 font-medium">
              ...
              <div className="space-y-3">
                {[
                  ...
                  <div key={row.label} className={`flex justify-between items-center py-2 ${row.sub ? 'pl-4 text-xs text-slate-500' : 'border-t border-slate-50 text-sm'}`}>
                ...
                <div className="border-t-2 border-slate-200 pt-3 flex justify-between">
                  <span className="font-bold text-slate-800">Utile Netto Stimato</span>
            ...
            {/* Grafico giornaliero */}
            <div className="bg-white rounded-2xl p-6 border border-slate-100 shadow-sm">
              <h3 className="text-base font-semibold text-slate-800 mb-4">Andamento giornaliero</h3>
      ...
      {activeTab === 'foodcost' && (
        <div className="space-y-6">
          <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
            {/* Categorie */}
            <div className="bg-white rounded-2xl p-6 border border-slate-100 shadow-sm">
              <h3 className="text-base font-semibold text-slate-800 mb-4">Fatturato per Categoria</h3>
              <ResponsiveContainer width="100%" height={220}>
                <RechartsPie>
            ...
            <div className="xl:col-span-2 bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden">
              <div className="flex items-center justify-between p-5 border-b border-slate-100">
                <h3 className="text-base font-semibold text-slate-800">Margini per Piatto</h3>
                <button onClick={exportFoodCost} className="flex items-center gap-1.5 text-xs text-orange-600 hover:text-orange-700 font-medium">
                    ...
                    {foodCost.slice(0, 15).map(item => (
                      <tr key={item.id} className="hover:bg-slate-50 transition-colors">
                        <td className="px-4 py-3">
                          <p className="font-medium text-slate-800 truncate max-w-[150px]">{item.name}</p>
                        ...
                        <td className="px-4 py-3 text-slate-600">{item.soldQty} pz</td>
                      </tr>
                    ))}
                    {foodCost.length === 0 && (
                      <tr><td colSpan={6} className="text-center text-slate-400 text-sm py-8">Collega gli ingredienti ai piatti nel Magazzino per vedere il food cost</td></tr>
                    )}
                  </tbody>
                </table>
              </div>
        ...
        <div className="space-y-6">
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            <div className="bg-white rounded-2xl p-5 border border-slate-100 shadow-sm text-center">
              ...
              <p className="text-2xl font-bold text-slate-800 mt-1">{formatCurrency(yearly?.totalRevenue || 0)}</p>
            </div>
            <div className="bg-white rounded-2xl p-5 border border-slate-100 shadow-sm text-center">
              <p className="text-sm text-slate-500">Mese Migliore</p>
              <p className="text-xl font-bold text-orange-600 mt-1">{yearly?.bestMonth?.monthName || '—'}</p>
              <p className="text-sm text-slate-400">{formatCurrency(yearly?.bestMonth?.revenue || 0)}</p>
            </div>
            <div className="bg-white rounded-2xl p-5 border border-slate-100 shadow-sm text-center">
              <p className="text-sm text-slate-500">Media Mensile</p>
              <p className="text-2xl font-bold text-slate-800 mt-1">{formatCurrency((yearly?.totalRevenue || 0) / 12)}</p>
            </div>
          </div>

          <div className="bg-white rounded-2xl p-6 border border-slate-100 shadow-sm">
            <h3 className="text-base font-semibold text-slate-800 mb-4">Fatturato Mensile {selectedYear}</h3>
              ...
              <tbody className="divide-y divide-slate-50">
                {(yearly?.months || []).map(m => (
                  <tr key={m.month} className="hover:bg-slate-50 transition-colors">
                    ...
                    <td className="px-4 py-3 text-slate-600">{m.orders}</td>
                    <td className="px-4 py-3 text-slate-600">{m.orders > 0 ? formatCurrency(m.revenue / m.orders) : '—'}</td>
                  </tr>
                ))}
              </tbody>
            </table>
