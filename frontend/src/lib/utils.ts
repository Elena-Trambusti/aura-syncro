...
export function formatCurrency(amount: unknown, currency = 'EUR'): string {
  return new Intl.NumberFormat(getIntlLocale(), { style: 'currency', currency }).format(moneyNumber(amount))
}
